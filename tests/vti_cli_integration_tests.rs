...
fn vti_cli_headless_addams2_reports_pcm_activity() {
    ...
    assert!(
        value > 0,
        "expected non-zero PCM activity, stdout: {stdout}"
    );
}
...
fn vti_cli_without_arguments_fails_with_usage() {
    let bin = env!("CARGO_BIN_EXE_vti-cli");
    let output = Command::new(bin).output().expect("failed to run vti-cli");

    assert!(
        !output.status.success(),
        "expected non-zero exit code without arguments"
    );
