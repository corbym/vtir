...
- **Refer to the original source.** The Pascal files (`trfuncs.pas`, `AY.pas`, etc.) are preserved in the `pascal/` directory at the project root. When porting a routine, read the original implementation first and translate it statement-by-statement before refactoring. Do not guess at intended behaviour.
...
- Port a **new** function from `pascal/trfuncs.pas` or `pascal/AY.pas` to Rust, **or**
...
|----------|--------------|-----|
| **Playback engine** | `pascal/trfuncs.pas` → `Pattern_PlayCurrentLine`, `Pattern_PlayOnlyCurrentLine` | AY register values per tick; any off-by-one is audible |
| **Chip clock / synthesiser logic** | `pascal/AY.pas` → `TSoundChip.Synthesizer_Logic_Q`, `Synthesizer_Logic_P` | Tone/noise/envelope counter progression drives PCM output |
| **Level table calculation** | `pascal/AY.pas` → `Calculate_Level_Tables` | Two-step rounding (scale then volume) produces different integers if collapsed; wrong tables → wrong PCM amplitude |
| **Song timing** | `pascal/trfuncs.pas` → `GetModuleTime`, `GetPositionTime`, `GetPositionTimeEx`, `GetTimeParams` | Used for seek/scrub; off-by-one causes wrong seek target |
| **Reverse note lookup** | `pascal/trfuncs.pas` → `GetNoteByEnvelope2`, `GetNoteByEnvelope` | Floating-point rounding; used in envelope editor |
| **Format parsers** (when ported) | `pascal/trfuncs.pas` → `PT32VTM`, `STC2VTM`, `SQT2VTM`, … | Binary → Module round-trip; wrong field decode is silent |
