...
fn build_open_options() -> JsValue {
    // { types: [{ description: "Tracker modules", accept: { "application/octet-stream": [...] } }], multiple: false }
    let exts = Array::new();
    ...
    exts.push(&JsValue::from_str(".sqt"));
    exts.push(&JsValue::from_str(".asc"));
    exts.push(&JsValue::from_str(".as0"));
    exts.push(&JsValue::from_str(".gtr"));
