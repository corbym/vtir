...
fn audio_player_starts_and_accepts_samples() {
    let player = vti_audio::AudioPlayer::start(44100).expect("failed to open audio device");
...
fn audio_player_fill_level_decreases_over_time() {
    ...
    let samples: Vec<StereoSample> = vec![
        StereoSample {
            left: 1000,
            right: 1000
        };
...
fn audio_player_diagnostics_show_callback_activity() {
    ...
    assert!(
        ...
        "audio callback should run after start"
    );
    assert!(
        after.pushed_samples >= before.pushed_samples + samples.len() as u64,
        "push counter should advance"
    );
    assert!(
        after.popped_samples > before.popped_samples,
        "callback should consume at least some samples"
    );
}
