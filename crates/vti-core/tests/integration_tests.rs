...
// ─── Smoke tests for new format parsers ──────────────────────────────────────
// Each parser must: (1) return Err on empty input, (2) return Ok on a minimal
// valid header (rather than panic).

use vti_core::formats::{asc, fls, gtr, sqt};
...
fn gtr_ok_on_minimal_header() {
    // 296-byte zeroed header (GTR_NumberOfPositions=0 → empty module)
    let data = [0u8; 296];
...
fn sqt_ok_on_minimal_header() {
    // 12-byte all-zero header: SQT_PositionsPointer=0 → first byte=0 → terminates immediately
    let data = [0u8; 12];
